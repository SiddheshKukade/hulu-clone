...
const Thumbnail = ({ result }, ref) => {
    ...
    return (
        <div ref={ref} className='group cursor-pointer p-2 transition duration-200 ease-in transform sm:hover:scale-105 hover:z-50'>
            <Image layout='responsive' alt={result?.title || result?.original_name}
                src={`${BASE_URL}${result?.backdrop_path || result.poster?.path}` || ` ${BASE_URL}${reuslt.poster_path}`} height={1080} width={1920} />
            <div className='p-2'>
                ...
                <p className='flex items-center opacity-0 group-hover:opacity-100 '>{result.media_type && `${result.media_type} : `} {result.release_date || result.first_air_dawa}
